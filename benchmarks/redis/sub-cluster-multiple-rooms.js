...
var TABLE_NAMES = [];
var NUM_ROOMS = 1000;
_.range(NUM_ROOMS).forEach(function (d) {
    TABLE_NAMES.push('room-' + d);
});
...
if(cluster.isMaster){
    ...
    var combinedRoomSubCount = 0;
    var roomSubCountPerWorker = {};
    ...
    _.each(workers, function (worker) {
        worker.on('message', function(message) {
            if (message.messageType === 'clientUpdate') {
                ...
                roomSubCountPerWorker[message.workerId] = message.workerRoomSubCount;
                combinedRoomSubCount = _.sum(_.values(roomSubCountPerWorker));
    ...
    setInterval(() => {
        ...
        logger.log('cluster-master', d3.format(',')(NUM_CPUS * NUM_CONNECTIONS) + ' Connections');
        logger.log('cluster-master', d3.format(',')(combinedRoomSubCount) + 
            ' total room subscriptions | %j', {
            roomSubCountPerWorker: _.values(roomSubCountPerWorker)
        });
    ...
    */
    ...
    setInterval(function sendInfoToMaster () {
        process.send({
            ...
            workerRoomSubCount: workerRoomSubCount,
    ...
    setTimeout(function() {
        async.eachLimit(
            ...
            function setupConnection (connectionIndex, cb) {
                ...
                for (var curIndex = (Math.random() * 30 | 0); curIndex >= 0; curIndex--) {
                    sub = TABLE_NAMES[Math.random() * NUM_ROOMS | 0];
